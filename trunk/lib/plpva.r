...
#
   if( fdattype=="integer" ){
     ...
     cdi <- cumsum(hist(z,c(min(z)-1,(xmin+.5):floor(xmax),xmax),plot=FALSE)$counts/nz)
     gof <- max(abs( fit - cdi ))
     pz  <- nz/N

     mmax <- 20*xmax
     pdf <- c(rep(0,xmin-1),(((xmin:mmax)^-alpha))/(zvec[I]-sum((1:(xmin-1))^-alpha)))
     ...
     for(B in 1:Bt){
       ...
       for(qm in 1:length(qmins)){
 	   ...
 	   # vectorized version of numerical calculation
           ...
           cdi <- cumsum(hist(zq,c(min(zq)-1,(qmin+.5):qmax,max(zq)+1),plot=FALSE)$counts/nq)
